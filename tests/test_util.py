...
def mock_package_resources(monkeypatch):
    ...
    monkeypatch.setattr(
        util.filesystem, "resource_listdir", lambda __, path: dir_listing[path]
    )
    monkeypatch.setattr(
        util.filesystem, "resource_stream", lambda __, path: file_contents[path]
    )
...
def test_load_pkg_resources_opens_correct_files():
    ...
    streams = util.open_resource_files(root_dir="conf.d", glob="*.service.toml")
